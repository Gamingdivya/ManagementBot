...
# from MahakRobot.modules.sql import warns_sql as warnssql
import MahakRobot.modules.sql.blacklist_sql as blacklistsql

# from MahakRobot.modules.sql import cust_filters_sql as filtersql
# import MahakRobot.modules.sql.welcome_sql as welcsql
import MahakRobot.modules.sql.locks_sql as locksql
import MahakRobot.modules.sql.notes_sql as sql

# from MahakRobot.modules.rules import get_rules
import MahakRobot.modules.sql.rules_sql as rulessql
from MahakRobot import EVENT_LOGS,OWNER_ID, SUPPORT_CHAT, dispatcher
from MahakRobot.__main__ import DATA_IMPORT
from MahakRobot.modules.connection import connected
from MahakRobot.modules.helper_funcs.alternate import typing_action
from MahakRobot.modules.helper_funcs.chat_status import user_admin
from MahakRobot.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("MahakRobot{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("MahakRobot{}.backup".format(chat_id), "rb"),
        caption="❍ *sᴜᴄᴄᴇssғᴜʟʟʏ ᴇxᴘᴏʀᴛᴇᴅ ʙᴀᴄᴋᴜᴘ*\n❍ ᴄʜᴀᴛ ➛ `{}`\n❍ ᴄʜᴀᴛ ɪᴅ ➛ `{}`\n❍ ᴏɴ ➛ `{}`\n\n❍ ɴᴏᴛᴇ ➛ ᴛʜɪs `MahakRobot-Backup` ᴡᴀs sᴘᴇᴄɪᴀʟʟʏ ᴍᴀᴅᴇ ғᴏʀ ɴᴏᴛᴇs 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("MahakRobot{}.backup".format(chat_id))  # Cleaning file
